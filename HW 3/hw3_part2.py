...
def move_pokemon(pos,direction,steps):
    '''
    moves the pokemon a defined amount of steps in user specified direction. 
...
if often <= 0:
    poke_list = []
    #if the often is less than or equal to 0 it will use an empty list since range can't incriment by 
else:
...
while i <= turns:
    ...
    if x == 'N' or x == 'S' or x == 'E' or x == 'W':
        ...
        if i in poke_list:
            ...
            poke_type = poke_type.upper()
            if poke_type == 'W':
            #for water type pokemon, the pokemon will win but cant move bc no direction
                print("{} wins and moves to {}".format(name, position))
                win_loss.append('Win')
            elif poke_type == 'G':
                #for ground type, the pokemon will lose but cant move bc no direction
                print("{} runs away to {}".format(name, position))
                win_loss.append('Lose')
            else:
                win_loss.append('No Pokemon')        
        #even if the pokemon isn't moving in NSEW it still counts as a turn and then should be 
        i+= 1
