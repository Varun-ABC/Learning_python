...
def tourist_pop(bears):
    '''
    given the population of bears it will calculate the populataion of tourists
    multiple branches based off of tourist data in responce to bear population
    will always have whole numbers as .5 of a tourists/ bear cannot exist
    sample output= 50000
    '''
...
def find_next (bears, berries, tourists):
    '''
    given the current population of bears, berries, and tourists it will calculate the 
    next year populataion of bears and berry area
    sample output:
    (4,633.5)
    '''
...
berry_list = []
tourists_list = []
#lists help me find the min and max easily 

tourist = tourist_pop(bears)
i = 1
while i <= 10:
    if i == 1:
        #first year is done independently because nothing needs to be calculated 
        #(except the toursist pop which is based on intial value for bears) 
        #and hence running the numbers through formula will mess them up
        ...
        #need new variables for bears and berries becuase after I run the code 
        #the new value for "berries" would replace the old one and mess up the code down the line so berries_new solves that,
        #ditto for bears
